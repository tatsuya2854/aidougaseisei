...
`python3 handoff/veo_generate.py --estimate` で同じ表が出ます。
Veoは**返ってくるクリップの秒数**で課金されます（8秒固定）。こちらで2〜3秒に切り詰めても
課金は8秒ぶんなので、8カットで **8秒 × 8本 = 64秒** が課金対象です。

| ルート | 8カット合計 | 可視ウォーターマーク |
|---|---|---|
...
- **Veoは有料ティアのキーが必要**です。ここは実測で確認済みで、**無料ティアのキーでも
  `--list-models` にはVeoが3つとも表示されます**。表示される＝使える、ではありません。
  実際に投げると `429 RESOURCE_EXHAUSTED`（"check your plan and billing details"）が返り、
  full / fast / lite のどれでも同じです。テキストモデルは同じキーで普通に通るので、
  キーの不備ではなく**Veoのクォータが0**という状態です。キーの所属プロジェクトで課金を
  有効にすれば解消します。なお429は生成前に弾かれるので課金は発生しません。
